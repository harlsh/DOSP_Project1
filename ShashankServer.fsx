...
let workers = 0L //fsi.CommandLineArgs.[2] |> int64
...
let worker (mailbox: Actor<_>) =
    let rec loop () =
        actor {
            ...
            match msg with
            | InitiateWorker -> mailbox.Sender() <! RequestJob
            | Job (str) ->
        ...
        actor {
            ...
            | RequestJob ->
                if stringFound then
                    ...
                    let randomSting = "shashankk.hayyal;" + ranStr () // generate a random string
            ...
            | RemoteJobRequest (remoteWorker) ->
                remoteWorker
                <! "Process," + (string k) + ",shashankk.hayyal;"
        ...
        let rec loop () =
            actor {
                ...
                if msg.CompareTo("RemoteRequestJob") = 0 then
                    ...
                    CoordinatorRef
                    <! RemoteJobRequest(mailbox.Sender())
                else
                    printfn "Success from remote server:"

                    CoordinatorRef
                    <! RemoteSuccess(message.[0], message.[1], mailbox.Sender())
